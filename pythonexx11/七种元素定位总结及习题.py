...
driver.find_element_by_css_selector('input[name="wd"]').send_keys(789)  # 如标签名没有任何符合，直接写标签名。 通过元素的标签来定位,因为重名的标签很多,所以在标签名后面指定一个属性
...
driver.find_element_by_css_selector('*[href="http://map.baidu.com"]').click()   # *如果出现在=前面.才是包含匹配,如果出现在本来应该写标签名的位置,代表任意标签.
